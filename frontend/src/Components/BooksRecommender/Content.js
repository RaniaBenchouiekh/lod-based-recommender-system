...
import { verifyAccess, webidToUri ,readInfoPerso, readRatedBooks, readSavedBooks, readGenresFile } from '../SolidBooks/Communication'
import loader from '../../Assets/oval.svg'
import axios from 'axios'
import './Welcome.scss'
import anonyme from '../../Assets/user.png'
import { readRatedMovies, readSavedMovies, readMoviesGenresFile } from '../SolidMovies/Communication';
...
class Content extends React.Component {
    ...
    async componentDidMount(){
        ...
        try{
            ...
            let jsonArrBooks = jsonArr1.concat(jsonArr2)

            console.log("profile books : ")
            console.log(jsonArrBooks)

            //Read Genre books  
            var booksGenres = await readGenresFile(uri)

            var gnrs = booksGenres
            ...
            try {
                ...
                jsonArrMovies = jsonArr1.concat(jsonArr2)

                console.log("profile movies : ")
                console.log(jsonArrMovies)

                //Read Genre movies
                moviesGenres = await readMoviesGenresFile(uri)
            ...
            let ratingsArray = await axios
                                .post('http://localhost:3005/lods/books', jsonRequest)
                                .then(res => {return res} )
                                .catch(err => {
                                    this.setState({ calculationError: true })
                                    console.error(err);
                                });   
            this.setState({ booksToRecommend : ratingsArray.data.data, loading2: false })
            console.log(ratingsArray.data.data)

            //save books in localStorage
            window.localStorage.setItem("booksRecommended", JSON.stringify(ratingsArray.data.data))
                            ...
                            <div className="RS-terminalBody">
                                {
                                    ...
                                    this.state.booksToRecommend.map(book=>{
                                        return <div key={book.resourceExt}>{book.resourceExt}</div>
                                    })
                                    ||
